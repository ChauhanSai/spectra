...
def _get_pipe():
    ...
    print(f"Loading {MODEL_ID} (local Gemma 3n pipeline)...", file=sys.stderr)
    _pipe = pipeline(
        "image-text-to-text",
        model=MODEL_ID,
...
def predict_label(pil_img) -> str:
    ...
    try:
        ...
        if out and len(out) > 0:
            ...
            if isinstance(msg, dict) and "generated_text" in msg:
                ...
                if isinstance(gen, str):
                    text = gen.strip()
                elif isinstance(gen, list) and gen:
                    last = gen[-1]
        ...
        if not _first_error_logged:
            ...
            print("Local Gemma 3n error (first occurrence):", e, file=sys.stderr)
        return "unknown"
    finally:
