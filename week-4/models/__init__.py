...
# Models that support security-policy-hardened prompts
PolicyPredictor = Callable[[Image.Image, str], str]

POLICY_REGISTRY: dict[str, PolicyPredictor] = {
    "gemma-3-local": local_gemma.predict_label_with_policy,
    "gemma-3-4b-it-local": local_gemma.predict_label_with_policy,
    "llama3.2-vision": local_llama32_vision.predict_label_with_policy,
    "llama-3.2-vision": local_llama32_vision.predict_label_with_policy,
    "qwen": qwen.predict_label_with_policy,
    "qwen2.5-vl-3b-instruct": qwen.predict_label_with_policy,
}


def get_policy_predictor(model_id: str) -> PolicyPredictor | None:
    model_id = (model_id or "").strip().lower()
    return POLICY_REGISTRY.get(model_id)
