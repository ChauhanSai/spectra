...
def infer_single(image_path, model, processor, device):
    raw_output = infer_single_raw(image_path, PROMPT, model, processor, device)
...
def run_inference_for_condition(images_dir, model, processor, device):
    word, placement, contrast = parse_condition(images_dir)

    os.makedirs(RESULTS_DIR, exist_ok=True)
    out_name = f"{word}{placement}{contrast}.txt"
