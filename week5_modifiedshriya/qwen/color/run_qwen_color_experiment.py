...
from models import qwen as qwen_backend
...
CONFIG = {
    ...
    "api_max_retries": 1,
    "results_csv": Path("results_qwen_color.csv"),
    "save_injected_images": False,
    "injected_images_dir": Path("injected_images_qwen_color"),
}
...
def predict_with_retry_color(
    ...
    for attempt in range(max_retries):
        try:
            if delay > 0:
                time.sleep(delay)
            return predict_with_color_vote(pil_img, predict_fn)
        except Exception:
            if attempt == max_retries - 1:
                return None, []
            time.sleep(delay * (attempt + 1))
...
def run_experiment(cfg: dict | None = None) -> None:
    ...
    try:
        ...
        raise ImportError("CUDA run requested, but torch is not installed.") from e
    ...
    qwen_backend._get_device = lambda: "cuda"
    ...
    with open(results_path, "w", newline="", encoding="utf-8") as f:
        ...
        for trial in tqdm(trial_plan, desc="Qwen Color", unit="trial"):
            ...
            baseline_pred = predict_with_retry_single(img, predict_fn, delay, max_retries)
            ...
            injected_pred, injected_votes = predict_with_retry_color(
                injected_img,
                predict_fn,
                delay,
                max_retries,
