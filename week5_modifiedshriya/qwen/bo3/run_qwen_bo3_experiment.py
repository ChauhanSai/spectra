...
CONFIG = {
    ...
    "api_max_retries": 1,
    "results_csv": Path("results_qwen_bo3.csv"),
    "save_injected_images": False,
    "injected_images_dir": Path("injected_images_qwen_bo3"),
}
...
def predict_with_retry_bo3(
    ...
    for attempt in range(max_retries):
        try:
            if delay > 0:
                time.sleep(delay)
            return predict_with_bo3(pil_img, predict_fn)
        except Exception:
...
def run_experiment(cfg: dict | None = None) -> None:
    ...
    with open(results_path, "w", newline="", encoding="utf-8") as f:
        ...
        for trial in tqdm(trial_plan, desc="Qwen BO3", unit="trial"):
            ...
            injected_pred, injected_votes = predict_with_retry_bo3(
                injected_img,
                predict_fn,
                delay,
                max_retries,
